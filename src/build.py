...
# ---------- 計算ドリル ----------
def cell(lbl, qty, amt, how=""):
    # how＝その欄の出し方（式）。段階1「誘導あり」だけで見せ、段階2・3では隠す（誘導のフェード A5）
    h = '<div class="how num">%s</div>' % how if how else ""
    return '<div class="bcell"><div class="lbl">%s</div><div class="qty num">%s</div>%s<div class="amt num">%s</div></div>' % (lbl, qty, h, amt)
def guide(t): return '<span class="how">%s</span>' % t   # 「（差引）」など、欄の中の案内語も段階2以降は隠す
...
DRILLS = [
 {"id":"D1","tag":"工業簿記","title":"総合原価計算・平均法","sub":"ボックス図を埋めて完成品原価まで（7ステップ）",
  ...
  "boxes": box("原料費のボックス（数量：個）", [cell("月初仕掛品","200個","¥84,000"), cell("完成品","1,600個",blank("matDone")), cell("当月投入","1,800個","¥756,000"), cell("月末仕掛品","400個",blank("matEnd"))])
          + box("加工費のボックス（数量：完成品換算量）", [cell("月初仕掛品","100個","¥90,000","200個×50%"), cell("完成品","1,600個",blank("convDone")), cell("当月投入",guide("（差引）"),"¥990,000"), cell("月末仕掛品",blank("convQty")+" 個",blank("convEnd"),"400個×50%")])
          + box("まとめ", [cell("月末仕掛品原価","",blank("endTotal"),"原料費＋加工費"), cell("完成品総合原価","",blank("final"),"原料費＋加工費")], one=True),
  "steps":[
 ...
 {"id":"D2","tag":"工業簿記","title":"総合原価計算・先入先出法","sub":"当月投入の換算量が鍵（5ステップ）",
  ...
  "boxes": box("原料費のボックス（数量：個）", [cell("月初仕掛品","200個","¥90,000"), cell("完成品","1,600個",guide("（差引）")), cell("当月投入","1,800個","¥756,000"), cell("月末仕掛品","400個",blank("matEnd"),"当月投入の単価×400個")])
          + box("加工費のボックス（数量：完成品換算量）", [cell("月初仕掛品","100個","¥45,000"), cell("完成品","1,600個",guide("（差引）")), cell("当月投入",blank("convIn")+" 個","¥1,020,000","完成＋月末−月初"), cell("月末仕掛品","200個",blank("convEnd"),"当月投入の単価×200個")])
          + box("まとめ", [cell("月末仕掛品原価","",blank("endTotal"),"原料費＋加工費"), cell("完成品総合原価","",blank("final"),"月初＋当月投入−月末")], one=True),
  "steps":[
 ...
 {"id":"D3","tag":"工業簿記","title":"標準原価計算・直接材料費差異","sub":"価格差異と数量差異を分析図で（6ステップ）",
  ...
  "boxes": box("標準と実際", [cell("標準消費量","",blank("stdQty")+" kg","1,000個 × 2kg"), cell("標準原価","",blank("stdCost"),"@500 × 標準消費量"), cell("実際消費量","",'<span class="num">2,100 kg</span>'), cell("実際原価","@480 × 2,100kg",'<span class="num">¥1,008,000</span>')])
          + box("差異分析図（縦：単価、横：数量）", [cell("価格差異","",blank("priceVar"),"（標準単価−実際単価）×実際消費量"), cell("価格差異の向き","",blank("priceDir")), cell("数量差異","",blank("qtyVar"),"標準単価×（標準消費量−実際消費量）"), cell("数量差異の向き","",blank("qtyDir"))]),
  "steps":[
 ...
 {"id":"D4","tag":"工業簿記","title":"直接原価計算・CVP分析","sub":"損益分岐点から安全余裕率まで（6ステップ）",
  ...
  "boxes": box("直接原価計算方式の損益計算書（当期）", [cell("売上高","2,500個 × @2,000",'<span class="num">¥5,000,000</span>'), cell("変動費","2,500個 × @1,200",'<span class="num">¥3,000,000</span>'), cell("貢献利益","1個あたり "+blank("cmUnit")+" 円",'<span class="num">¥2,000,000</span>',"販売単価−変動費"), cell("貢献利益率","",blank("cmRate"),"貢献利益 ÷ 売上高"), cell("固定費","",'<span class="num">¥1,600,000</span>'), cell("営業利益","",'<span class="num">¥400,000</span>')], one=True)
          + box("CVP分析", [cell("損益分岐点売上高","",blank("bepSales"),"固定費 ÷ 貢献利益率"), cell("損益分岐点販売量","",blank("bepQty")+" 個","固定費 ÷ 1個あたり貢献利益"), cell("目標利益達成売上高","",blank("targetSales"),"（固定費＋目標利益）÷ 貢献利益率"), cell("安全余裕率","",blank("safety"),"（売上高−損益分岐点）÷ 売上高")]),
  "steps":[
